...
fn f_score<'a>(actual: &[&'a str], predicted: &[&'a str]) -> BTreeMap<&'a str, f64> {
    ...
    // Count TP, FP, and FN using every actual/predicted pair
...
fn main() {
    ...
    // Perform kfold cross val. Keep track of results for final averages
    let mut tree_macro_fs: Vec<f64> = Vec::new();
    let mut knn_macro_fs: Vec<f64> = Vec::new();
    for i in 0..kfold {
        // Figure out indices for cross val.
